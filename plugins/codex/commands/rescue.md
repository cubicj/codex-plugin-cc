...
argument-hint: "[--background|--wait] [--read-only|--write] [--resume|--resume-thread <thread-id>|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [what Codex should investigate, solve, or continue]"
...
- Leave `--resume`, `--resume-thread <thread-id>`, and `--fresh` in the forwarded request. The subagent handles that routing when it builds the `task` command.
- The subagent must preserve the user's task text verbatim inside a controller envelope whose first line is `[Claude-Orchestrated Task]`. The envelope must state that live in-session communication is in Korean, controller-consumed output is in English, and routine questions should be handled by stating an assumption and proceeding or recording uncertainty in the final report.
